...
var QueryGenerator = {
  ...
  */
    ...
    if (options.include) {
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (through) {
          ...
          var joinInclude = self.joinInclude({
            model: mainModel,
            subQuery: options.subQuery,
            include: include,
            groupedLimit: options.groupedLimit
          });
          joinQueries.joinAttributes.push(joinInclude.joinAttribute);
          joinQueryItem = ' ' + joinInclude.sql;
        }
      ...
      options.include.filter(function (include) {
        ...
        var joinAttributes = joinQueries.joinAttributes.map(function(joinAttribute) {
          return mainTableAs + '.' + self.quoteIdentifiers(joinAttribute);
        });
        subQueryAttributes = _.union(subQueryAttributes, joinAttributes);
  ...
  joinInclude: function(options) {
    ...
    return {
      joinAttribute: fieldLeft,
      sql: joinType + this.quoteTable(tableRight, asRight) + ' ON ' + joinOn
    };
  },
